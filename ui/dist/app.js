(() => {
  ...
  (function() {
    ...
    const state = {
      ...
      sendTransport: null,
      recvTransport: null,
      localProducers: /* @__PURE__ */ new Map(),
      consumers: /* @__PURE__ */ new Map(),
      producerToConsumer: /* @__PURE__ */ new Map(),
      pendingTransportRequests: /* @__PURE__ */ new Map(),
      pendingConnectRequests: /* @__PURE__ */ new Map(),
      pendingProduceRequests: /* @__PURE__ */ new Map(),
      pendingConsumeRequests: /* @__PURE__ */ new Map(),
      subscribedProducers: /* @__PURE__ */ new Set(),
...
`;
    ...
    function attachRemoteStream(consumer) {
      ...
      element.addEventListener("error", (evt) => {
        log(
          `Remote ${consumer.kind} element error (${consumer.id}): ${evt?.message || evt?.type || "unknown"}`
        );
      });
    ...
    function handleTransportConnected(msg) {
      ...
      let pending = requestKey ? state.pendingConnectRequests.get(requestKey) : null;
      if (!pending) {
    ...
    function handleProduced(msg) {
      let requestKey = msg.requestId;
      let pending = requestKey ? state.pendingProduceRequests.get(requestKey) : null;
      if (!pending) {
    ...
    function handleConsumed(msg) {
      let requestKey = msg.requestId;
      let pending = requestKey ? state.pendingConsumeRequests.get(requestKey) : null;
      if (!pending) {
    ...
    function handleProducersList(msg) {
      log(
        `Received producers list (${(msg.producers || []).length}) for room ${msg.room}`
      );
    ...
    joinForm.addEventListener("submit", async (event) => {
      ...
      const wsUrl = wsInput.value.trim() || `ws://${location.hostname || "localhost"}:3000`;
